...
export class NotificationService {
  ...
  async sendQueueNotification(email: string, username: string, usernamePattern: string): Promise<void> {
    try {
...
This notification was sent because you registered for queue notifications on PrismaX AI Reminder Service.
  ...
  async sendWelcomeEmail(email: string, username: string, usernamePattern: string): Promise<void> {
    try {
      const { data, error } = await this.resend.emails.send({
        from: process.env.EMAIL_FROM || 'PrismaX Reminder <noreply@yourdomain.com>',
        to: [email],
...
Thank you for using PrismaX AI Reminder Service!
      ...
      if (error) {
        ...
        // Don't throw error for welcome email, as registration should still succeed
      } else {
